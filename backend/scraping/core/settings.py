...
try:
    ...
    # Repo root = parents[3]:
    # settings.py → core → scraping → backend → joola-intel-nextjs (repo root)
    _root = Path(__file__).resolve().parents[3]
    # Prefer repo-root .env (shared by backend + analytics_backend).
    # Fall back to legacy .env so old checkouts still work.
