...
1. Create `backend/scraping/sources/<channel>/scrape_<thing>.py` exposing
   `def run(ctx: dict) -> int:` (return the number of rows upserted).
2. Add the import path to `MODULE_STEPS` in `backend/scraping/run.py`.
...
5. Smoke test:
   ```bash
   python -m backend.scraping.run --module <channel> --dry-run
   python -m backend.scraping.run --module <channel> --brands joola
   ```

## Adding a new local-Playwright product brand

Edit `backend/scraping/sources/products/scrape_catalog_local.py`:
