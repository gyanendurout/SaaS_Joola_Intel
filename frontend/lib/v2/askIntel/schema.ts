...
export const WHITELISTED_TABLES: Record<string, TableSpec> = {
  ...
  reddit_mentions: {
    ...
    columns: [
      ...
      { name: 'post_title', type: 'text', description: 'Post title (column is post_title, NOT title).' },
      { name: 'content_text', type: 'text', description: 'Post body text (column is content_text, NOT body).' },
      { name: 'upvotes', type: 'integer', description: 'Upvote count (column is upvotes, NOT score).' },
      { name: 'post_url', type: 'text', description: 'Permalink (column is post_url, NOT url).' },
      { name: 'is_crisis', type: 'boolean', description: 'Crisis flag.' },
      { name: 'velocity_per_hour', type: 'numeric', description: 'Viral velocity in upvotes/hour.' },
      { name: 'is_removed', type: 'boolean', description: 'True if post was removed/deleted.' },
  ...
  tiktok_videos: {
    ...
    columns: [
      ...
      { name: 'text', type: 'text', description: 'Caption text (column is "text", NOT "caption" or "description").' },
      { name: 'view_count', type: 'integer', description: 'Views (column is view_count, NOT play_count).' },
      { name: 'like_count', type: 'integer', description: 'Likes.' },
      { name: 'comment_count', type: 'integer', description: 'Comment count.' },
      { name: 'share_count', type: 'integer', description: 'Share count.' },
      { name: 'posted_at', type: 'timestamptz', description: 'Posted timestamp.' },
      { name: 'is_crisis', type: 'boolean', description: 'Crisis flag.' },
    ],
    allowedFilters: {
      brand_id: ['eq', 'in'],
      view_count: ['gt', 'gte', 'lt', 'lte'],
