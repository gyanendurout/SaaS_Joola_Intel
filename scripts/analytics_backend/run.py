...
Usage:
  ...
  python -m scripts.analytics_backend.run --module statistics
  python -m scripts.analytics_backend.run --module statistics --source granger
  python -m scripts.analytics_backend.run --module all --dry-run
"""
...
# Format identical to scripts/scraping/run.py: list of parallel groups.
MODULE_STEPS: dict[str, list[list[tuple[str, str]]]] = {
    "marts": [
        # Phase A — calendar must exist before timeseries can join on it
        [("scripts.analytics_backend.marts.refresh_calendar", "run")],
        # Phase B — helpers can run in parallel (each writes its own table)
        [("scripts.analytics_backend.marts.refresh_helpers", "run")],
        # Phase C — main mart joins everything together
        [("scripts.analytics_backend.marts.refresh_timeseries", "run")],
    ],
    "statistics": [
        # All four can run in parallel; each reads from joola_timeseries_daily
        # and writes to analysis_results with its own `kind` tag
        [("scripts.analytics_backend.statistics.correlation_scan", "run"),
         ("scripts.analytics_backend.statistics.cross_correlation", "run"),
         ("scripts.analytics_backend.statistics.changepoints",      "run"),
         ("scripts.analytics_backend.statistics.granger",           "run")],
    ],
}
